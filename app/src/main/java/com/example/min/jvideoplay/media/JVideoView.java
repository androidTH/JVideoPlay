 ...
 * {@link #(boolean)}
 */
public class JVideoView extends SurfaceView implements PlayerController{
    private static final String TAG = JVideoView.class.getSimpleName();
...
//            getHolder().setFixedSize(videoWidth, videoHeight);
    ...
    private void openVideo() {
        ...
        try {
            ...
            mMediaPlayer.setOnBufferingUpdateListener(mBufferingUpdateListener);//注册一个回调函数，在网络视频流缓冲变化时调用。
            ...
            mMediaPlayer.prepareAsync();//预处理播放器为播放做准备，方法是异步执行。设置完数据源和显示的Surface后，
            // 你需要调用prepare()或prepareAsync()。对于流，你应该调用prepareAsync()，立即返回，而不是阻塞，直到足够的数据被缓冲。
...
//            Log.e(TAG, "IjkVideoView onVideoSizeChanged: videoWidth = "+isVideoSizeChanged);
            Log.e(TAG,"IjkVideoView onVideoSizeChanged: videoWidth = " + videoWidth + ",videoHeight = " + videoHeight);
    ...
    IMediaPlayer.OnPreparedListener mPreparedListener = new IMediaPlayer.OnPreparedListener() {
        public void onPrepared(IMediaPlayer mp) {
            ...
            Log.e(TAG, "IjkVideoView onPrepared: mVideoWidth = " + mVideoWidth + ",mVideoHeight = " + mVideoHeight);
...
//        	Log.i("zl","SurfaceHolder.Callback surfaceChanged");
            mSurfaceWidth = w;
            mSurfaceHeight = h;
            Log.e(TAG, "surfaceChanged() mSurfaceWidth = " + mSurfaceWidth + ",mSurfaceHeight = " + mSurfaceHeight);
...
//                            toggleMediaControlsVisiblity();
    ...
    public long getCurrentPosition() {
        if (isInPlaybackState()) {
            ...
            Log.i("kooMEDIA", "IjkVideoView position = " + position);
            return position;
        }
        return 0;
    }
